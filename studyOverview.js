(function (global, factory) {
    ...
    function captureRequiredVariables() {
      this.settings.modules.forEach(function (module) {
        module.variables = Array.from(new Set(d3.merge(module.results.map(function (result) {
          return result.subset ? result.subset.map(function (subset) {
            return subset.key;
          }) : [];
        }))));
      });
    ...
    function checkDataSpecification(data) {
      ...
      var datasets;
      if (Array.isArray(data) && !data[0].hasOwnProperty('data')) datasets = [{
        data: data
      }];else datasets = data;
      datasets.forEach(function (dataset) {
        dataset.variables = Object.keys(dataset.data[0]);
        if (dataset.hasOwnProperty('spec')) dataset.module = _this.settings.modules.find(function (module) {
          ...
          dataset.module = matches.find(function (match) {
            return match.proportion === d3.max(matches, function (match) {
